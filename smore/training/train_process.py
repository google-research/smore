...
def test_step_wikikgv2(model, args, evaluator, test_dataloader, result_buffer, train_step, device, phase):
    ...
    # print(model.entity_embedding.embedding[:3, :2])
    # print(model.relation_embedding.embedding[:3, :2])
    # print(model.relation_feat[:3, :2])
    # print(model.entity_feat.embed[:3, :2])
    # print(model.feature_mod.entity_proj.weight[:3, :2])
    # print(model.feature_mod.relation_proj.weight[:3, :2])
    # exit(-1)
    with torch.no_grad():
        for negative_sample, queries, queries_unflatten, query_structures, easy_answers, hard_answers in tqdm(test_dataloader, disable=(not args.print_on_screen) or rank):
            ...
            # easy_answers = [easy_answers[i] for i in idxs]
            # hard_answers = [hard_answers[i] for i in idxs]
            ...
            # all_top10_cand.append(top10_cand)
            step += 1
