...
class ComplexDistFunc(torch.autograd.Function):
    ...
    def forward(ctx, entity_embed, re_embed, im_embed):
        ...
        with torch.no_grad():
            ...
            extlib.complex_dist_forward(entity_embed, re_embed, im_embed, dst)
            return dst
    ...
    def backward(ctx, grad_out):
        with torch.no_grad():
            ...
            extlib.complex_dist_backward(grad_out, entity_embed, re_embed, im_embed, grad_entity, grad_re, grad_im)
            return grad_entity, grad_re, grad_im


def complex_sim(entity_embed, re_embed, im_embed):
    if entity_embed.dim() != re_embed.dim():
        ...
        for i in range(re_embed.shape[1]):
            ...
            d = ComplexDistFunc.apply(entity_embed, re, im)
            l_dist.append(d)
        d = torch.stack(l_dist, dim=1)
        return d
    else:
        assert entity_embed.dim() == 3 and re_embed.dim() == 3 and im_embed.dim() == 3
        return ComplexDistFunc.apply(entity_embed, re_embed, im_embed)


def naive_complex_sim(entity_embedding, query_re_embedding, query_im_embedding):
...
def test_complex():
    ...
    l1 = naive_complex_sim(entity, re, im)
