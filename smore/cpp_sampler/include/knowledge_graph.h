...
public:
    ...
    void load_from_numpy(void* triplets, size_t n_triplets, const bool has_reverse_edges);
...
private:
    ...
    void load_edges_from_file(const std::string& fname, const bool has_reverse_edges, std::vector< std::tuple<Dtype, Dtype, Dtype> >& edges);
    void build_kg_from_edges(std::vector< std::tuple<Dtype, Dtype, Dtype> >& edges);
};
