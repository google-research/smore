...
template<typename Dtype>
void ISampler<Dtype>::sample_batch_entities(bool weighted, int num_samples, py::array_t<long long, py::array::c_style | py::array::forcecast> _entities)
{
    long long* entities = _entities.mutable_unchecked<1>().mutable_data(0);
    if (!weighted) {
        for (int i = 0; i < num_samples; ++i)
            entities[i] = rand_int((Dtype)0, this->kg->num_ent);
    } else {
        for (int i = 0; i < num_samples; ++i)
            entities[i] = this->am_in->draw_sample();
    }
}
...
template<typename Dtype>
int ISampler<Dtype>::next_batch(py::array_t<long long, py::array::c_style | py::array::forcecast> _positive_samples,
                                ...
                                py::array_t<long long, py::array::c_style | py::array::forcecast> _query_args)
{
    ...
    size_t strides = _query_args.strides(0) / sizeof(long long);
    ...
    for (int i = 0; i < batch_size; ++i)
    {
        ...
        if (i == 0)
            q_type = cur_sample->q_type;
        else
            assert(q_type == cur_sample->q_type);
        ...
        for (size_t j = 0; j < cur_sample->query_args.size(); ++j)
            query_args[strides * (size_t)i + j] = cur_sample->query_args[j];
	delete cur_sample;
    }
    if (share_negative) {
        negative_samples = _negative_samples.mutable_unchecked<2>().mutable_data(0, 0);
