...
template<typename Dtype>
int QueryTree<Dtype>::get_num_args()
{
    if (node_type == QueryNodeType::entity)
        return 1;
    else if (node_type == QueryNodeType::entity_set) {
        auto& ch = this->children[0];
        int ch_num_args = ch.second->get_num_args();
        return ch_num_args + 1;
    } else {
        int num = 0;
        for (auto& ch : this->children)
            num += ch.second->get_num_args();
        if (node_type == QueryNodeType::union_set)
            num += 1;
        return num;
    }
...
template<typename Dtype>
Batch1pQuerySample<Dtype>::Batch1pQuerySample(int _q_type) : q_type(_q_type)
{
    this->heads.clear();
    this->sample_weights.clear();
    this->relations.clear();
    this->tails.clear();
    this->negative_samples.clear();
}

template<typename Dtype>
Batch1pQuerySample<Dtype>::~Batch1pQuerySample()
{

}

template class Batch1pQuerySample<unsigned>;
template class Batch1pQuerySample<uint64_t>;
